    ...
    teachers.forEach(teacher => {
      if (teacher.sections) {
        teacher.sections.forEach(section => {
          ...
          sectionData.totalReviews += stats.totalReviews;
          sectionData.totalRating += parseFloat(stats.overallAverage);
        });
      }
    });

    // Calculate average ratings for sections
    const sectionsArray = Array.from(sectionMap.entries()).map(([section, data]) => {
      const averageRating = data.teachers.length > 0 
  ...
  return (
    <div className="all-sections-container">
      ...
      <div className="sections-grid">
        {sectionStats.map(({ section, teachers: sectionTeachers, averageRating, totalReviews }) => {
            ...
            <div key={section} className={`section-card ${ratingColor}`}>
              <div className="section-header">
                <div className="section-title">
                  ...
                  <span className="teacher-count">{sectionTeachers.length} teachers</span>
                </div>
